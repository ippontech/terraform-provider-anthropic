...
This is a Terraform provider built with [HashiCorp Terraform Plugin Framework](https://developer.hashicorp.com/terraform/plugin/framework) v1.19.0. It is a MemoryStore fork of [ippontech/terraform-provider-anthropic](https://github.com/ippontech/terraform-provider-anthropic); the Go module path remains `github.com/ippontech/terraform-provider-anthropic`, while the Terraform Registry address is `registry.terraform.io/MemoryStore/anthropic`.
...
- `main.go` — entry point; serves the provider at `registry.terraform.io/MemoryStore/anthropic`
...
internal/
  ...
  retry/           — Skills 429/rate-limit HTTP client plus multipart upload with 5xx retry (import alias: provretry)
...
- `anthropic_agent` (`internal/services/agents/agent_resource.go`) — manages Managed Agents (create/read/update/delete). Fork-only fields: `model_effort` (`low`/`medium`/`high`/`xhigh`/`max`) and optional `multiagent` coordinator roster (`type = "coordinator"`; entries `agent` with required `id`, or `self` which forbids `id`/`version`; at most one `self`; agent IDs unique; `version` optional+computed so the API can pin latest). Custom object/list validators live next to the schema. Map both fields in the agent and agents data sources as well.
