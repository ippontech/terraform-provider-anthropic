...
- `internal/acctest/acctest.go` exports `ProtoV6ProviderFactories`, `PreCheck` (standard API key), `PreCheckAdmin` (admin API key), and `PreCheckOAuth` (`ANTHROPIC_AUTH_TOKEN`, `t.Skip`s when unset) used by all acceptance tests. `PreCheckOAuth`-gated tests (currently `anthropic_service_account` and `anthropic_service_account_workspace`) run **locally only**: CI's `testacc` job carries the standard and admin API keys but no durable org:admin OAuth token (WIF bootstrap pending, [#137](https://github.com/ippontech/terraform-provider-anthropic/issues/137)).
