...
**`subcategory` must never be empty.** It controls how the Terraform Registry groups resources and data sources in its sidebar. An empty subcategory causes the resource/data source to appear at the top level, ungrouped, which clutters the navigation. Always assign a meaningful category (e.g. `"Agents"`, `"Messages"`, `"Models"`).

`docs/` files are auto-generated by `make generate` — **never edit them directly**. To set the subcategory, create a template under `templates/` for each resource and data source:

```
templates/
├── resources/
│   └── <name>.md.tmpl       # sets subcategory + embeds example + schema
└── data-sources/
    └── <name>.md.tmpl
```
...
Without a template, `make generate` will auto-generate the doc with `subcategory: ""`. **Whenever you add a new resource or data source, always create its template file at the same time.**
