...
func TestBuildNetworkingUnion_Limited(t *testing.T) {
	...
	// The "type" discriminator must be set, otherwise the API rejects the
	// request with "networking.type: Field required".
	if string(union.OfLimited.Type) != "limited" {
		t.Errorf("expected limited networking type %q, got %q", "limited", string(union.OfLimited.Type))
	}
...
// ---------------------------------------------------------------------------
// mcpServerURLForState tests
// ---------------------------------------------------------------------------

func TestMCPServerURLForState(t *testing.T) {
	cases := []struct {
		name    string
		...
		{"trailing slash appended by API is preserved", types.StringValue("https://mcp.example.com"), "https://mcp.example.com/", "https://mcp.example.com"},
		{"exact match kept", types.StringValue("https://mcp.example.com/oauth"), "https://mcp.example.com/oauth", "https://mcp.example.com/oauth"},
		{"genuinely different uses API value", types.StringValue("https://old.example.com"), "https://new.example.com/", "https://new.example.com/"},
		{"null planned (import) uses API value", types.StringNull(), "https://mcp.example.com/", "https://mcp.example.com/"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := mcpServerURLForState(tc.planned, tc.apiVal)
